...
namespace SimpleOJ.Service
{
    ...
    public class JwtTokenService : IJwtTokenService
    {
        ...
        public string GenerateToken(UserToken userInfo,DateTime now)
        {
            ...
            client.Set(userInfo.Id, now.ToFileTimeUtc().ToString(),new TimeSpan(0,0,30,0));//refresh token存入redis
            return jwtToken;
        }
        ...
        public string UpdateToken(UserToken userInfo)
        {
            ...
            //refresh token
            //无条目时创建，有条目时覆盖
            client.Set(userInfo.Id, now.ToFileTimeUtc().ToString(), new TimeSpan(0, 0, 30, 0));
            //access token
        ...
        {
            var json = DeocdeJwtToken(token, secret);
            switch (json)
            {
                case "error":
                {
